...
每个角色必须包含以下字段，**从剧本中找不到的字段填"未知"或"未提及"，不要遗漏任何角色**：

- name: 角色姓名（无名角色标注"配角1""路人2"等）
- role: 角色定位（主角/配角/反派/路人等）
- gender: 性别（从台词/动作/称呼推断男/女/未知）
- age: 年龄（如剧本未明确，写"未提及"，不要留空）
- height: 身高（如剧本未明确，写"未提及"）
- body_type: 体型（如剧本未明确，写"未提及"）
- hairstyle: 发型（如剧本未明确，写"未提及"）
- hair_color: 发色（如剧本未明确，写"未提及"）
- face_shape: 脸型（如剧本未明确，写"未提及"）
- eyes: 眼睛（如剧本未明确，写"未提及"）
- skin: 肤色（如剧本未明确，写"未提及"）
- outfit: 服装（如剧本未明确提及，写"未提及"）
- accessories: 饰品（无则填"无"）
- personality: 性格（从台词语气/动作/身份推断）
- appearances: 出场场次（如"第1场"）

**关键要求：即使剧本只有少量描述，也必须为每个角色输出完整的16个字段，绝不可返回空数组。**
...
只输出JSON数组，不要其他文字。"""
    ...
    raw_chars, raw_scenes, raw_props = await asyncio.gather(
        call_minimax(chars_prompt, f"剧本：\n{req.script[:6000]}"),
        call_minimax(scenes_prompt, f"剧本：\n{req.script[:6000]}"),
        call_minimax(props_prompt, f"剧本：\n{req.script[:6000]}"),
    )
