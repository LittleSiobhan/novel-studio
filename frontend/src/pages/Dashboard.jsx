...
const S = {
  bg: '#050810', card: 'rgba(8, 16, 40, 0.9)', cardBorder: 'rgba(56,189,248,0.12)',
  inputBg: 'rgba(8, 16, 40, 0.8)', accent: '#38bdf8', accentLight: '#7dd3fc',
  accentGlow: 'rgba(56, 189, 248, 0.1)', text: '#e2e8f0', textMuted: '#64748b',
  error: '#f87171', success: '#34d399', warn: '#f59e0b',
}

function Card({ children, style }) {
  return <div style={{ background: S.card, backdropFilter: 'blur(24px)', border: `1px solid ${S.cardBorder}`, borderRadius: 16, padding: 24, boxShadow: '0 4px 24px rgba(0,0,0,0.3), 0 0 40px rgba(56,189,248,0.03), inset 0 1px 0 rgba(255,255,255,0.03)', ...style }}>{children}</div>
}
...
export default function Dashboard() {
  ...
  const downloadScript = async () => {
    if (!script.trim()) { setError('没有剧本可下载'); return }
    try {
      const r = await fetch(`${API}/download-script`, {
        method: 'POST', headers: authHeaders(),
        body: JSON.stringify({ script, title: selected?.name || '剧本' }),
      })
      const d = await r.json()
      if (d.path) {
        // 创建下载链接
        const a = document.createElement('a')
        a.href = `${API}/read-script-file?path=${encodeURIComponent(d.path)}`
  ...
  const handleExtractAssets = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API}/extract-full-assets`, {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ script, title: selected?.name, model: assetModel }),
        signal: controller.signal,
      })
      clearTimeout(timer)
      const d = await r.json()
  ...
  return (
    <div style={{ background: S.bg, minHeight: '100vh', color: S.text }}>
      {/* 顶栏 */}
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(5,8,16,0.95)', backdropFilter: 'blur(20px)', borderBottom: `1px solid ${S.cardBorder}`, boxShadow: '0 1px 20px rgba(56,189,248,0.05)' }}>
              ...
              {projects.map(p => (
                <div key={p.id} onClick={() => selectProject(p)}
                  style={{ padding: '10px 12px', borderRadius: 10, cursor: 'pointer',
                    background: selected?.id === p.id ? 'rgba(56,189,248,0.12)' : 'rgba(8,16,40,0.6)',
                    border: `1.5px solid ${selected?.id === p.id ? S.accent : 'transparent'}`, transition: 'all 0.2s' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: 8 }}>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: selected?.id === p.id ? S.accentLight : S.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', transition: 'color 0.2s' }}>{p.name}</div>
                  ...
                  <div style={{ marginTop: 6 }}>
                    <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 500,
                      background: p.status === 'assets_done' ? 'rgba(52,211,153,0.1)' : p.script ? 'rgba(52,211,153,0.08)' : 'rgba(0,0,0,0.03)',
                      color: p.status === 'assets_done' ? '#10b981' : p.script ? S.accent : S.textMuted }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <Card>
              ...
              <div style={{ marginTop: 12 }}>
                <div style={{ fontSize: 11, color: S.textMuted, marginBottom: 8, letterSpacing: '0.05em' }}>剧本风格（可多选）</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {[
                    ...
                    const isActive = scriptStyles.includes(s.label)
                    return (
                      <button key={s.label} onClick={() => {
                        if (isActive) {
                          // 至少保留一个
                          if (scriptStyles.length === 1) return
                          setScriptStyles(scriptStyles.filter(x => x !== s.label))
                        } else {
                          setScriptStyles([...scriptStyles, s.label])
                        }
                      }}
                        style={{
                          padding: '5px 12px', borderRadius: 20, fontSize: 12, cursor: 'pointer',
                          border: `1px solid ${isActive ? (s.label === '短剧爆款' ? '#f97316' : S.accent) : 'rgba(56,189,248,0.15)'}`,
                          background: isActive ? (s.label === '短剧爆款' ? 'rgba(249,115,22,0.12)' : 'rgba(56,189,248,0.12)') : 'transparent',
                          color: isActive ? (s.label === '短剧爆款' ? '#fb923c' : S.accentLight) : S.textMuted,
                          transition: 'all 0.2s',
                          fontFamily: 'inherit',
                        }}>
                ...
                {scriptStyles.length > 0 && (
                  <div style={{ marginTop: 8, fontSize: 11, color: S.textMuted }}>
                    已选：<span style={{ color: S.accentLight }}>{scriptStyles.join(' + ')}</span>
                  </div>
                )}
              </div>
